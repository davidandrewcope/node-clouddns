...
Record.prototype = {
	...
	updateRecord : function updateRecord(details) {
	    ...
	    var reqOpts = {
		  	  method   : 'PUT'
		  	, uri      : this._domain._client.dnsUrl('domains', this._domain.id, 'records', this.id) 
		  	, client   : this._domain._client
		  	, body     : this._getProperties()
		  }
		  
	    this._domain._client.rackspace(reqOpts, callback, function (body) {
	...
	, removeRecord : function removeRecord() {
	    ...
	    this._domain._client.rackspace(reqOpts, callback, function (body, res) {
